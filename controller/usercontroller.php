...
if (isset($_POST['updateprofileimage'])) {
...
} elseif (isset($_POST['UpdatePayScaleRecords'])) {
    $req_data_for = SECURE($_POST['req_data_for'], "d");
    ...
    //attandance records
} elseif (isset($_POST['AttandanceRecords'])) {
    ...
    $CheckUserStatus = CHECK("SELECT * FROM users where UserEmailId='" . $_POST['UserEmailId'] . "'");
    if ($CheckUserStatus != null) {
        $Response = false;
        $Msg = "<b>" . $_POST['UserPhoneNumber'] . "</b> or <b>" . $_POST['UserEmailId'] . "</b> is already exits!";
    } else {
        ...
        if ($Users == true) {
            ...
            //get user id
            $UserAddressUserId = FETCH("SELECT * FROM users ORDER BY UserId DESC limit 1", "UserId");
            ...
            $Response = INSERT("user_addresses", $Address);
            $Msg = "Unable to save user details at the moment!";
        } else {
            $Response = false;
            $Msg = "Unable to save user record at the moment!";
        }
    }
    RESPONSE($Response, "User details is saved successfully!", $Msg);
}
